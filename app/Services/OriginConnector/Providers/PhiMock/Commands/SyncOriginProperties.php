...
namespace App\Services\OriginConnector\Providers\PhiMock\Commands;
...
class SyncOriginProperties extends Command
{
    ...
    protected function initializeConnectorSettings($account)
    {
        ...
        if (!$connectorSettings->settings || !$connectorSettings->settings['mock_api_url']) {
            $this->output->newLine();
            $this->comment('Let\'s setup the Mock CMS connector settings... To apply the default value (shown in brackets) hit enter.');
            $settings = [];
            $settings['mock_api_url'] = $this->ask('What is the Mock CMS API URL?');
            $settings['mock_api_key'] = $this->ask('What is the Mock CMS API key?');
    ...
    protected function syncProperties($account)
    {
        try {
            ...
            $property = Origin::getPropertyInfo();
            if (is_null($property)) {
